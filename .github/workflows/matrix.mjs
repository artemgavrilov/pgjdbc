...
// The oauth auth method was added in PostgreSQL 18, and the validator the test setup installs
// is built for it as well (pg-oidc-validator-pgdg18.deb, see
// docker/postgres-server/scripts/entrypoint.sh).
matrix.imply({oauth: {value: ['smoke', 'all']}}, {pg_version: greaterThanOrEqual('18')})
// The Keycloak and PostgreSQL containers the test needs are only started on the Docker-based
// (Linux) setup.
matrix.imply({oauth: {value: ['smoke', 'all']}}, {os: {value: 'ubuntu-latest'}})
// OAUTHBEARER must run over TLS (RFC 7628, section 3), so OAuth rows need ssl=yes.
matrix.imply({oauth: {value: ['smoke', 'all']}}, {ssl: {value: 'yes'}})
...
include.forEach(v => {
  ...
  if (v.oauth === 'all') {
      // Redirect connections of the default user to OAuth, see TestUtil.addOAuthProperties

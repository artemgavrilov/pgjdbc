...
### Added
* feat: add OAuth Bearer authentication (OAUTHBEARER SASL, [RFC 7628](https://www.rfc-editor.org/rfc/rfc7628)) for PostgreSQL 18 and later, where the server side `oauth` authentication method was introduced. The application supplies the token, either directly through the new `oauthToken` connection property or through the new `org.postgresql.plugin.OAuthTokenProvider` SPI, whose implementation is named by `oauthTokenProviderClassName` and called once per connection attempt with an `OAuthTokenRequest` carrying `oauthIssuer`, `oauthClientId`, `oauthClientSecret` and `oauthScope`. The driver does not contact the issuer itself, so the OAuth discovery exchange is left for a later release. `requireAuth` accepts `oauth`, and when a server offers OAUTHBEARER alongside SCRAM-SHA-256, setting either token property selects OAUTHBEARER and `password` goes unused; `requireAuth=scram-sha-256` keeps the password path. `channelBinding=require` rules OAUTHBEARER out, since the driver has no channel binding to offer for it. The token is sent only over a connection to a verified server, as RFC 7628, section 3 requires: TLS with `sslmode=verify-ca` or `verify-full`, or GSS encryption, and `sslmode=require` is not enough because it encrypts without checking the server certificate. The new `oauthAllowInsecureConnection=true` lifts that requirement for development and testing and logs a warning instead, and a non-HTTPS `oauthIssuer` is rejected for the same reason unless `oauthAllowInsecureIssuer=true` [Issue #3816](https://github.com/pgjdbc/pgjdbc/issues/3816) [PR #4251](https://github.com/pgjdbc/pgjdbc/pull/4251)

### Fixed
* fix: `requireAuth` no longer demands `gss` on every GSS-encrypted connection. Establishing GSS encryption was taken as the authentication that happened, so `requireAuth=oauth` or `requireAuth=scram-sha-256` with `gssEncMode=require` was refused even though the client had authenticated with that method, and `requireAuth=gss` was refused when the server asked for a Kerberos handshake without GSS encryption. As in libpq, an authentication exchange the client completed meets `requireAuth` on its own, and GSS encryption stands in for an AuthenticationGSS request only where the server sends none. A server that authenticates the connection without asking the client for anything is still refused unless `none` is allowed, GSS-encrypted or not [PR #4251](https://github.com/pgjdbc/pgjdbc/pull/4251)
...
### Security
* fix: `oauthToken`, `oauthClientSecret` and `sslpassword` no longer reach the log. `BaseDataSource.getUrl()` built its query string from every connection property that was set, and `Driver.connect` and `BaseDataSource.getConnection` log a URL at `FINE` on both success and failure, so a credential held in a property was written to the log. `getUrl()` now omits every property that `PGProperty.isSensitive()` reports, `getConnection` passes those values to the driver directly, and the URLs the driver logs have the same values replaced with `***`. `password` was never affected, since `BaseDataSource` holds it in its own field rather than among the properties. Callers that passed `getUrl()` to `DriverManager.getConnection` and relied on it carrying `sslpassword` must now supply that credential themselves [PR #4251](https://github.com/pgjdbc/pgjdbc/pull/4251)
